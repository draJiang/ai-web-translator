...
8. 每次状态切换（开启/还原）content script 都会给 background 发一条 `STATE_CHANGED` 消息，background 用 `chrome.action.setIcon` 按 `tabId` 切换该标签页的工具栏图标；标签页开始加载新文档（整页刷新/跳转）时图标会自动重置为默认状态。
...
- SPA 内容切换的自动重新扫描只在**同一个文档**内有效（前端路由、AJAX 换内容）；如果是整页刷新或跳转到新 URL（浏览器发起新的文档加载），content script 会重新注入、状态归零，B1 模式不会跨页面记忆，需要重新点一次「转为 B1 英文」。
